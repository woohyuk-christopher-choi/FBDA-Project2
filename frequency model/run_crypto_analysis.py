...
cryptocurrency markets using high-frequency data from Binance.
...
Data Source: Binance (15 major cryptocurrencies)
Frequencies: 5m, 15m, 30m, 1h, 1d

Usage:
    python run_crypto_analysis.py

Output:
    - crypto_analysis_results.csv: Detailed results for all methods/frequencies
...
def analyze_single_frequency(
    ...
    min_obs = CONFIG['min_observations'].get(frequency, 10000)
    ...
    n_periods = 0
    min_obs_required = int(window_obs * 0.3)
    ...
    for i in range(len(rebalance_dates) - 1):
        ...
        est_returns = assets_returns.iloc[start_idx:rebal_idx + 1]
        ...
        hold_returns = assets_returns.loc[hold_mask]
...
def run_full_analysis():
    ...
    for freq in CONFIG['frequencies']:
        result_df = analyze_single_frequency(freq, data_dir)
...
def print_comprehensive_summary(detailed_df: pd.DataFrame, summary_df: pd.DataFrame):
    ...
    for freq in CONFIG['frequencies']:
        ...
        freq_minutes = FREQUENCY_MINUTES.get(freq, 1)
        obs_per_day = 1440 // freq_minutes
        window_obs = obs_per_day * window_days
